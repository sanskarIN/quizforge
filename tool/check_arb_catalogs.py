...
def validate_catalog(
    ...
    for key in sorted(catalog):
        ...
        if key.startswith("@"):
            ...
            if not isinstance(metadata, dict):
                problems.append(
                    CatalogProblem(path, f"metadata {key!r} must be an object")
                )
    ...
    if template_keys is not None:
        ...
        for key in extra:
            problems.append(
                CatalogProblem(path, f"message {key!r} is not present in template")
            )
...
def run(
    root: Path,
    template_relative: str = "lib/l10n/app_en.arb",
) -> list[CatalogProblem]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate QuizForge ARB localization catalogs."
    )
