...
final class _ImportExportPageState extends State<ImportExportPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final String formatLabel = _format == _BankFormat.json ? strings.json : strings.csv;
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: <Widget>[
            ...
            Semantics(
              label: strings.exportPreview(formatLabel),
              child: Container(
            ...
            if (_lastResult != null) ...<Widget>[
              ...
              _ImportSummary(result: _lastResult!, strings: strings),
            ],
            const SizedBox(height: AppSpacing.xxl),
            const Divider(),
            const SizedBox(height: AppSpacing.xl),
            Text(strings.localBackup, style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: AppSpacing.sm),
                ...
                OutlinedButton.icon(
                  onPressed: () async {
                    final ClipboardData? data =
                        await Clipboard.getData(Clipboard.kTextPlain);
                    if (data?.text != null) {
                      _backupController.text = data!.text!;
                    }
                  },
                  icon: const Icon(Icons.content_paste),
                  label: Text(strings.paste),
  ...
  Future<void> _import() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).importSaveFailed)),
        );
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _restoreLocalBackup() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).backupRestoreSuccess)),
        );
      }
    } on Object catch (error) {
      widget.controller.logger.error(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).backupRestoreFailed)),
        );
      }
    } finally {
      if (mounted) {
...
final class _ImportSummary extends StatelessWidget {
  const _ImportSummary({required this.result, required this.strings});
