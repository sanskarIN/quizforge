...
void main() {
  group('QuizForgeController persistence ordering', () {
    test(
      'failed active-profile persistence keeps current profile selected',
      () async {
    ...
    test(
      'failed profile creation preference save rolls back new profile',
      () async {
        final AppDatabase database = AppDatabase(NativeDatabase.memory());
        addTearDown(database.close);
        final _FakeProfilePreferences profilePreferences =
        ...
        final List<PlayerProfile> storedProfiles = await database
            .loadProfiles();
    ...
    test(
      'failed active-profile deletion preference save leaves profile intact',
      () async {
        final AppDatabase database = AppDatabase(NativeDatabase.memory());
        addTearDown(database.close);
        await database.upsertProfile(
    ...
    test(
      'failed settings persistence keeps in-memory settings unchanged',
      () async {
        final AppDatabase database = AppDatabase(NativeDatabase.memory());
        addTearDown(database.close);
    ...
    test(
      'partial reset failure reloads a consistent controller state',
      () async {
        final AppDatabase database = AppDatabase(NativeDatabase.memory());
        addTearDown(database.close);
        final _FakeSettingsStore settingsStore = _FakeSettingsStore();
